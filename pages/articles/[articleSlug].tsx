  ...
  return (
    <div>
      ...
      >
        ...
        <h1 style={{ fontSize: '36px', textAlign: 'center', marginBottom: '3px' }}>{Title}</h1>
        <h2 style={{ fontSize: '24px', textAlign: 'center', marginBottom: '7px' }}>{`By: ${Author}`}</h2>
        <div>
          {Blocks.map(({ Type: BlockType, Object }: { Type: string; Object: any }, idx) => {
            if (BlockType === 'Markdown') {
              ...
              return (
                <div key={`${BlockType}-${idx}`}>
                  <MultipleChoice {...Object} {...restProps} />
                  <hr style={{ margin: '16px 0' }} />
                </div>
              );
            } else {
